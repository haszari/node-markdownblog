...
exports.processMongoArticle = function(mongoItem) {
  ...
  fakeArticle.date = mongoItem.date || new Date();
  fakeArticle.published = mongoItem.published != undefined ? mongoItem.published : false;
...
exports.getArticle = function(slug, includeUnpublished, callback) {
  ...
  mongodb.connect(app.mongoConnectionString, function(err, db) {
    ...
    var collection = db.collection('cbr_content');
      ...
      var findSpec = { slug: slug };
      if (!includeUnpublished) {
        findSpec.published = true;
      }

    collection.findOne(findSpec, function(err, item) {
      if (item) {
...
exports.getArticlesByTag = function(tag, includeUnpublished, callback) {
  ...
  mongodb.connect(app.mongoConnectionString, function(err, db) {
    ...
    var collection = db.collection('cbr_content');
    var findSpec = { 'tags' : { $in: [tag] } };
    if (!includeUnpublished) {
      findSpec.published = true;
    }
    var spec = { $query: findSpec, $orderBy: { date: -1 } };

    // TODO should limit this to first N and not use toArray in case there are squillions
    collection.find(findSpec).sort({ date: -1 }).toArray(function(err, items) {
...
exports.getArticles = function(pageNumber, includeUnpublished, callback) {
  ...
  mongodb.connect(app.mongoConnectionString, function(err, db) {
    ...
    var findSpec = includeUnpublished ? { } : { published: true };
    var spec = { $query: findSpec, $orderBy: { date: -1 } };

    collection.find(findSpec, options).sort({ date: -1 }).toArray(function(err, items) {
